...
import {
  MapPin, Phone, Mail, Clock, Shield, Zap, Star,
  Send, ChevronDown, Facebook, Twitter,
} from "lucide-react";
...
export default function Contact() {
  ...
  const hours = settings ? `${settings.openTime} – ${settings.closeTime}` : "08:00 – 19:00";
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {infoCards.map((item, i) => (
              ...
              >
                ...
                {item.href ? (
                  <a href={item.href} className="font-black text-[#0a0f2e] hover:text-blue-700 transition-colors block text-[15px]">
                    {item.value}
                  </a>
                ) : (
                  <p className="font-black text-[#0a0f2e] text-[15px]">{item.value}</p>
                )}
            ...
            <FadeIn direction="left" className="lg:col-span-2 space-y-5">
              ...
              <div className="space-y-3 pt-2">
                {[
                  ...
                  { icon: Star,   title: "5-Star Service",          desc: "Consistently top-rated by our customers." },
                ].map(({ icon: Icon, title, desc }) => (
                  <div key={title} className="flex gap-4 items-start bg-gray-50 rounded-2xl border border-gray-100 p-4">
                    <div className="h-10 w-10 rounded-xl bg-[#0a0f2e] flex items-center justify-center flex-shrink-0">
                      <Icon className="h-5 w-5 text-blue-300" />
                    </div>
                    <div>
                      <p className="font-black text-[#0a0f2e] text-[13px] mb-0.5">{title}</p>
